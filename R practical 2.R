...
sales_data[order(sales_data$Price,sales_data$City ) , ]
...
aggregate(Quantity ~ Producr, data = sales_data, sum)
...
# 1. If discount > 10 mark "High Discount".
sales_data$DiscountTag <-
  ifelse(sales_data$Discount > 10, "High Discount","low discount")
...
result <- aggregate(DiscountAmount ~ City, data = sales_data, sum)
result[which.max(result$DiscountAmount), ]
